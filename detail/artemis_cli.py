...
if len(sys.argv) != 6:
  helper = 'usage: python3 _detail_artemis_cli.py action courseid exercise username password \nactions currently include "deadline"'
  raise helper
...
token = authenticate(sysargs["username"], sysargs["password"])
...
action_dispatch[action](sysargs["exercise"])
exit(0)